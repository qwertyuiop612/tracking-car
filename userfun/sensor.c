...
float last_trace = 0;
float trace_filtered = 4.5f; // EMA滤波后的trace，初始为中心值
int flag = 0;
...
float sensor_detect()
{
    ...
    if (get_gpio_state(SENSOR_GRP_SNESOR_7_PORT, SENSOR_GRP_SNESOR_7_PIN)) {sensor[7] = 8; i++;} else sensor[7] = 0;

    if (sensor[2] && sensor[3] && sensor[4] &&sensor[5]) //检测到标志线，标记置1
    {
        flag = 1;
    }
    ...
    else
    {
        ...
        raw_trace = (last_trace > 4.5f) ? 9.0f : 0.0f; // LLM 看到对称值
    }

    // EMA低通滤波，α=0.7（传感器二值信号，轻滤即可）
    trace_filtered = trace_filtered * 0.3f + trace * 0.7f;
